    ...
    public static void fromJsonToCollection() {
        if (filePath != null) {
            try (Reader reader = new FileReader(filePath)) {
                ...
                List<SpaceMarine> spaceMarines = gson.fromJson(reader, new TypeToken<List<SpaceMarine>>(){}.getType());
                if (spaceMarines.size() > 0) for (SpaceMarine spaceMarine: spaceMarines) { CollectionManager.addJsonObject(spaceMarine); }
